...
MigraGuard can run automatically on GitHub Pull Requests to block merges when risky migrations are detected and post inline review comments.

### 1. Add Workflow File

Create `.github/workflows/migraguard.yml` in your repository:
...
jobs:
  scan:
    ...
    steps:
      ...
      - name: Run MigraGuard
        uses: your-org/migraguard@main # Replace with the actual action path
```

### 2. Configuration

Create `.migraguard.yml` in your repository root to configure behavior:

```yaml
# Only scan these files (default: **/*.sql)
migration_pattern: "migrations/*.sql"

# Fail the CI check if these severities are found (default: high, critical)
blocking_severities:
  - "critical"
  - "high"
```
...
You can preview exactly what the CI gate will flag before opening a PR:

```bash
migraguard ci-check --diff-against main
```

### 4. PR Feedback Example

| PR Review Comment |
|-------------------|
| **Severity:** CRITICAL<br>**Rule:** MG003<br><br>DROP TABLE legacy_oauth_tokens is irreversible. All data in the table will be permanently lost once this migration is committed.<br><br>*Suggested fix:* Before dropping, archive the data... |

---

## Roadmap

The following features are **not** included in this phase (Phase 2) and are planned for future phases:
...
### Done (Phase 2)
- ~~**CI Integration** — GitHub Actions native integration with PR annotations~~
